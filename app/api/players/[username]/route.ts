...
export async function POST(request: Request, { params }: { params: { username: string } }) {
  try {
    ...
    console.log("Attempting to connect to backend:", `${API_BASE_URL}/players/${username}`)
    ...
    return NextResponse.json(
      {
        ...
        backend_url: `${API_BASE_URL}/players/${params.username}`,
...
export async function GET(request: Request, { params }: { params: { username: string } }) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch player from external API", details: errorData },
        { status: response.status },
      )
    }
    ...
    console.error(`Error in GET /api/players/${username}:`, error)
